...
- RETUNED DEFAULTS (engine.js `DEFAULT_CONFIG`, mirrored in config.js `DEFAULT_STAGE_MS`): stageIntervalMs 2000, spellAlongMs 1100, tierEvery 12, deepCutEvery 15, heatMaxRevealed 1. (`?stage=`/`?spell=` dev overrides + the DevTuner sliders still work; `?stage=` now clamps up to 12000.)
