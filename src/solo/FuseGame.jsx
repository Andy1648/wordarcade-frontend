...
function FuseInner({ data, createEngine, adapter, onExit }) {
  ...
  useEffect(() => {
    if (g.phase === 'over') {
      ...
      if (!winRecordedRef.current) {
        ...
        setWinsEarned(
          recordRound({ mode: 'fuse', wordsAccepted: g.engine.state.wordsSolved, weightedWords: g.luckyWeighted })
        );
      }
    } else {
  ...
  const winsTally = awardWins({ mode: 'fuse', wordsAccepted: s.wordsSolved, weightedWords: g.luckyWeighted });
