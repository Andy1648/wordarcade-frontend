...
import { awardWordXp, loadProgress } from '../progress/xp.js';
...
export function useRunMode() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (left <= 0) {
        ...
        // secondsLeft is the deadline-derived clock (fix/run-short-height) — 0 at a natural end.
        ...
        emitRunRound({
          ...
          cleared: !fumbled && score >= wall, stackIds: state.stackIds, secondsLeft: left,
        }, track);
  ...
  // feat/run-telemetry: one `run_over` per run, on the same transition into 'over' the bank
  // uses. A ref (not state) guards the once-only so a re-render / StrictMode double-effect
  // can't double-fire; the phase never leaves 'over' (a new run remounts the hook). `level`
  // is the player's menu level.
