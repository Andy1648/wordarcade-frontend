...
export default function Homepage({ onSelectGame, onCreateRoom, onJoinRoom, onCredits }) {
  ...
  // One shared ref for the wrap: drives BOTH the background parallax (writes
  // --px/--py) and the click ripple (appends rings into it).
  ...
  // the cursor is still the lerp settles, so the field reads clean. Disabled
  ...
  // an interactive element (card / button / link / input), spawn one expanding-
  // and-fading ring at the cursor and remove it on animationend. The ring is
  ...
  function handleBgPointerDown(e) {
    if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    if (e.target.closest('.game-card, button, a, input, [role="button"]')) return;
    const wrap = wrapRef.current;
    if (!wrap) return;
  ...
  return (
    <div className="homepage-wrap" ref={wrapRef} onPointerDown={handleBgPointerDown}>
      {/* Dim cursor-parallax depth shapes (behind everything). Near-invisible at
          rest; they drift as the pointer moves (rAF-lerped --px/--py), nearer
          layers travelling more. Decorative, inert. */}
