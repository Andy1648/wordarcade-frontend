...
export default function GameCard({ game, onSelect, onHover, topper }) {
  ...
  // 3D cursor-tilt: the card leans toward the pointer as it crosses it, eased by
  // a CSS transition on .game-card-tilt, and springs back to flat on leave. rAF-
  ...
  function applyTilt() {
    ...
    el.style.setProperty('--ry', `${(-ny * MAX_TILT).toFixed(2)}deg`); // invert so the face tips toward the cursor
  }
  ...
  return (
    ...
    >
      ...
      {/* Tilt layer: carries the 3D cursor-lean so the inner card keeps its own
          scale/beat transforms. perspective lives on the wrap. */}
      <div
