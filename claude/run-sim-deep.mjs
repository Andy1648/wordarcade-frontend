...
function playRound(rng, stack, knobs, ctx, mode) {
  ...
  // DESIGNED honours SHORT FUSE's wprMul (fewer words); LIVE never reads it.
  if (!LIVE) attempts = Math.round(attempts * knobs.wprMul);
  ...
  const luckyOdds = LIVE ? LUCKY_ODDS : knobs.luckyOdds; // LIVE oracle is fixed 1/40
...
const META = {
  ...
  'lexicographer':  { cat: 'offense', safety: 1 },
...
if (winRuns.length === 0) {
  ...
  for (const r of single.slice(0, 8)) {
