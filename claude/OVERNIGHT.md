...
**1. THE RUN's economy pays 2.5-6.2x under its own target.** The repo's own
`claude/run-econ-sim.mjs`, run unmodified, fails its own acceptance check today:
**155 / 189 / 245 wins/min at 12 / 15 / 20 wpm against the shipped 625-963 band** (0.16-0.39x),
blowing the <=2x spread gate at all three (6.23x / 5.08x / 3.94x). Bisected to
**`c180f90` (deck rebalance)**, which changed what a round SCORES without re-fitting the divisor
that prices it. Reproduce:

```
node claude/run-econ-sim.mjs
```

FIXED on **`fix/run-payout-rate`** (divisor 10 -> 2.5): 618 / 758 / 978 wins/min, spreads
1.56x / 1.54x / 1.56x. §4a.
...
| `fix/run-payout-rate` | `1c23753` | https://wordarcade-frontend-mag38fc7t-beenchilling.vercel.app |
| `fix/willchange-gate` | `983d9f9` | (added after this doc was first written) |
| `docs/block-state-4` | `a902149` | (docs only — carries this file) |
| backend `fix/blitz-data` | `c832299` | **LOCAL ONLY — never pushed** |

Two branches were added after the overnight run itself and fix things it found:
`fix/run-payout-rate` (the economy re-fit, §4a) and `fix/willchange-gate` (the blind gate and
the two non-compositor infinite animations, §4b-i).

All frontend branches verified in sync with origin by `git ls-remote`. `release/prod-1`,
...
**MAGNITUDE, CORRECTED.** An earlier draft of this doc said "~20-40x low", quoting a refutation
agent that RE-PRICED the payout on a 30s clock at 8-9 words/round. That was not a measurement of
the sim and it should not have been the headline. The sim is the repo's own acceptance instrument
and it imports the real `runWinsPayout` (`run-econ-sim.mjs:7`, used at `:37`), so its output is
the operative number:

```
$ node claude/run-econ-sim.mjs
  @12 words/min: RUN ≈ 155 wins/min (0.25x blitz, 0.16x chain) → new spread 6.23x ✗ >2x
  @15 words/min: RUN ≈ 189 wins/min (0.30x blitz, 0.20x chain) → new spread 5.08x ✗ >2x
  @20 words/min: RUN ≈ 245 wins/min (0.39x blitz, 0.25x chain) → new spread 3.94x ✗ >2x
```

**155 / 189 / 245 wins/min against the 625-963 band = 0.16-0.39x, a 2.5-6.2x shortfall**, failing
the <=2x spread gate at every throughput. `+0 WINS` was observed live on a casual round-2 death.

**FIXED on `fix/run-payout-rate`.** The divisor moved 10 -> 2.5 (now named `RUN_WINS_DIVISOR`),
chosen by sweeping 10 -> 1.5: five candidates put @15 wpm inside the band with every spread <=2x,
and 2.5 minimises the miss at both extremes (618 vs 625 lo = -1.1%; 978 vs 963 hi = +1.6%) with
the flattest spread of any candidate. Deck and wall untouched. Note NO divisor can put all three
rates inside the band — 12->20 wpm spans 1.58x while the band spans 1.54x, so that is geometry,
not a tuning failure. `src/runMode/runPayoutRate.test.js` now asserts the sim's acceptance at
`npm test`, and was verified to FAIL on the regression (reverting to /10 fails 3 of its 4 tests).

The sim also PRINTED the wrong formula for two tunes running — "cumulative/100 x roundReached/10"
long after the divisor had moved off 100. It now prints `RUN_WINS_DIVISOR` directly.
